...
## 🚀 新版本 v1.12.0 更新！
[![示範影片](http://img.youtube.com/vi/EgaxJ9mNg4U/0.jpg)](https://www.youtube.com/watch?v=EgaxJ9mNg4U)

### ⚡️ 功能特點
- **自訂背景圖片**：支援透過上傳圖片或 URL 設定側邊欄背景，並可調整透明與模糊度。
- **全新設定面板 UI**：採用手風琴式收合設計，讓設定項目分類更清晰，介面更清爽。
- **自訂主題顏色**：自由調整背景、強調色與文字顏色，打造個人專屬風格。
- **垂直分頁**：完整顯示分頁標題，不再被縮減成細小的圖示。
- **分頁群組**：完美整合 Chrome 原生分頁群組，同步顯示名稱與顏色。
- **書籤整合**：在統一的面板中管理所有的分頁與書籤。
- **關連分頁 (Linked Tabs)**：開啟書籤時自動建立關連，避免開啟重複的分頁。
- **跨視窗管理**：直接在側邊欄管理所有 Chrome 視窗的分頁，並支援全域搜尋。
- **動態渲染技術**：即使擁有數千個書籤也能維持流暢的操作體驗。
- **無障礙快捷鍵**：支援 `F2` 快速重新命名與 `Delete` 鍵快速刪除項目。
...
### 📚 閱讀清單與 RSS
您的個人文章策展中心，就在側邊欄裡。
- **Chrome 閱讀清單整合**：與 Chrome 原生的閱讀清單同步，無縫「稍後再讀」功能。
- **RSS 訂閱**：訂閱任何 RSS 訂閱源，新文章自動加入您的閱讀清單。
- **智慧去重**：基於Hash的過濾機制，確保不會重複加入項目。
- **排序選項**：依日期（新至舊／舊至新）或標題排序。
- **手動抓取**：使用「立即抓取」按鈕立刻拉取最新文章。
- **批次清除**：一鍵移除所有已讀項目。
