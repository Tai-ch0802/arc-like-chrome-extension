...
- **새로워진 설정 UI**: 아코디언 방식의 레이아웃을 도입하여 설정 항목이 더 깔끔하고 조직적으로 관리됩니다.
- **사용자 정의 테마 색상**: 배경색, 강조색, 텍스트 색상을 자유롭게 변경하여 개성 있는 외관을 연출하세요.
- **수직 탭**: 전체 페이지 제목을 한눈에 볼 수 있으며, 더 이상 작은 아이콘으로 압축되지 않습니다.
- **탭 그룹**: Chrome 탭 그룹과 완벽하게 통합되어 색상과 이름을 동기화합니다.
- **북마크 통합**: 탭과 북마크를 하나의 패널에서 관리할 수 있습니다.
- **연결된 탭**: 북마크를 열 때 자동으로 "링크"를 생성하여 중복 탭이 생기는 것을 방지합니다.
- **전역 창 관리**: 모든 창의 탭을 관리하고 전역 검색을 통해 즉시 탐색할 수 있습니다.
- **동적 렌더링**: 수천 개의 북마크가 있어도 부드러운 성능을 보장합니다.
- **접근성 단축키**: `F2`로 이름 변경, `Delete`로 항목 삭제 등 빠른 기능을 지원합니다.
...
### 📚 읽기 목록 및 RSS
사이드바에서 바로 나만의 기사 큐레이션 허브를 만나보세요.
- **Chrome 읽기 목록 통합**: Chrome의 기본 읽기 목록과 동기화되어 원활한 "나중에 읽기" 기능을 제공합니다.
- **RSS 구독**: 원하는 RSS 피드를 구독하면 새 기사가 읽기 목록에 자동으로 추가됩니다.
- **스마트 중복 제거**: 해시 기반 필터링으로 여러 번 가져오더라도 중복 항목이 생기지 않도록 보장합니다.
- **정렬 옵션**: 날짜(최신순/오래된순) 또는 제목별로 정렬하여 빠르게 액세스할 수 있습니다.
- **수동 가져오기**: "지금 가져오기" 버튼으로 최신 기사를 즉시 불러올 수 있습니다.
- **일괄 삭제**: 클릭 한 번으로 읽은 항목을 모두 삭제할 수 있습니다.
