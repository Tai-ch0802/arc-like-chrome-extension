...
| 檔案 | 職責 | 層 |
|---|---|---|
| `modules/newswire/normalizer.js` | 純函式：各源 raw payload → `NewsEvent`（`parseTree/parseFj/parseAlpaca/parseJin10`＋共用 `sanitizeEvent`：欄位裁剪、長度上限、URL scheme 驗證） | utils 級純邏輯 |
...
| `modules/newswire/adapters.js` | 四源 adapter 工廠（見 §5.2）：連線/認證/訂閱/心跳/解析委派 normalizer；每源獨立重連狀態機（指數退避＋抖動，上限 60s；連續失敗 10 次 → DEGRADED）；jin10 含 REST 輪詢備援模式 | 根層（SW 專用） |
...
| Key | Area | Shape | 說明 |
|---|---|---|---|
| `newswireConfig` | **local** | `{ schemaVersion:1, sources:{ tree:{enabled,updatedAt}, fj:{enabled,updatedAt}, alpaca:{enabled,updatedAt}, jin10:{enabled,mode:'wss'\|'rest',updatedAt} }, rules:{ p0:string[], p1:string[], mute:string[], updatedAt }, prefs:{ notificationsEnabled:true, syncKeys:false, updatedAt } }` | 可同步部分的本機工作副本（**不含 keys**）。字串陣列每項 ≤64 字元、每組 ≤50 項 |
...
// createJin10Adapter({secretKey, mode}, hooks)  // mode:'wss'(M0 查證後定稿)|'rest'(alarms 輪詢)
...
| 補漏 | Tree：重連自帶 history replay（過 L1 去重）;Benzinga 型 `replay` 不適用（不接）;FJ/Alpaca/金十：v1 接受 ≤30s 缺口（REST 回補列 v1.5） | 誠實揭露於 PRD NFR |
...
| 階段 | 內容 | 驗收 |
|---|---|---|
| **M0 查證（N1 前置，1–2 晚）** | 以 scratch script（非入版控）dump 四源真實 payload：Tree（免 key）、FJ Free、Alpaca paper、**金十官方 WSS 端點/認證/心跳實測**（文件為登入後 SPA，需人工取規格；若 WSS 規格不可得 → jin10 v1 以 REST 輪詢出貨，manifest 內 `mode:'rest'`） | schema 校正 normalizer fixtures；金十路線拍板 |
...
| 風險 | 對策 |
|---|---|
| 金十官方 WSS 規格未經實測（文件登入牆） | M0 查證為 N1 前置硬條件；REST 輪詢為已定備援，兩模式同 adapter 介面 |
