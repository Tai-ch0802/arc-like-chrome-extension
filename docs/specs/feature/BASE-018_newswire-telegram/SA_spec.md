...
| ID | BASE-018 |
| 分級 | **T2**（Phase 2：SA） |
| 狀態 | **v1.6 — TG2c（options 登入 UI）**；TG1/TG1-live/TG2a/TG2b 已合併（#194/#195/#196/#197）。登入卡片就緒,真登入/收訊待 TG3 手動矩陣 |
| 日期 | 2026-07-23 |
...
| **TG2c** | options tg 卡片(兩態):`tgLoginController`(login/logout/resolveChannel,可測)＋`renderTgCard`(api_id/hash 引導、`client.start` 登入彈驗證碼/2FA、session 存/登出撤銷、頻道管理＋策展 7 頻道＋仿冒確認、session 風險告示 modal)＋i18n 27 key×14。**無新權限**(既有廣域 host_permissions `*://*/*` 已涵蓋 Telegram web DC＋offscreen/storage),故不另立 PERMISSIONS.md。真登入列 TG3｜本 PR |
...
| v1.6 | 2026-07-24 | **TG2c**：options Telegram 登入 UI。`tgLoginController`(login=client.start／logout=client.logOut 遠端撤銷／resolveChannel,DI 可測);`renderTgCard` 兩態(未登入 api_id/hash+phone+登入;已登入身份+enable+頻道管理+策展 7 頻道+登出撤銷);session 風險告示 modal;`modalManager.showPrompt` 加 inputType(2FA 遮罩)+message;i18n 27 key×14。無新權限(既有廣域 host_permissions 已涵蓋)。真登入列 TG3 | Tai / Claude 協作 |
