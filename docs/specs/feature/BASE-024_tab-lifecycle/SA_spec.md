...
| v1.3 | 2026-07-28 | Claude Code | P3 落地：snooze 建立正式改走 `lifecycle:snooze` 訊息（v1.0 的 D2「sidepanel 直接 store+alarm」被 v1.2 單一寫者取代、就此廢止）；引擎 ack 以顯式 persisted 旗標回報——serialize 鏈為保命吞錯，若直接 ack 會讓寫入失敗也回 ok:true、呼叫端誤關分頁（實作時抓到並封死，fail-CLOSED 與 routing claim 的 fail-open 方向相反，本功能的資料遺失防線要求如此）。options 設定落於 features 區尾端兩列（FR-5.01）。全功能（P1–P3）交付完畢 |
| v1.2 | 2026-07-28 | Claude Code | P2 落地：v1.1 前提**定案為 (a) 單一寫者**——sidepanel 對兩 storage key 的所有寫入走 runtime message 進 SW serialize 鏈（`lifecycle:removeArchived`/`clearArchived`/`wakeNow`/`cancelSnooze`；P3 將補 `lifecycle:snooze`），sidepanel 只讀。cancelSnooze＝轉封存（archive-add 先於 snooze-remove，URL 恆存在）；喚醒/取消一併 `alarms.clear`。另：§8.2 漏列 `sectionOrder.test.mjs`（UT 也硬編碼五區塊）——實作時抓到補上 |
