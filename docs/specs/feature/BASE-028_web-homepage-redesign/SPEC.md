...
附帶釐清(#234 review 提出):出血項即使是 `min()` 三者中最小,mockup 也**不會**貼齊視窗邊緣 — 公式加的是 `(100vw - 1180px) / 2`,而 `.container` 的 24px 右內距從未被補償,實測五種出血項勝出的視窗,間距都是常數 24px(有捲軸時約 16px)。因此初版的 `border-radius: … 0 0 …`(砍掉右側圓角)不對應任何實際畫面,拿掉是修正而非取捨。若日後要真出血,需同時補這 24px 並讓直角只在出血項勝出時生效。
