...
✨ Новые функции
- Связывание вкладок и закладок: Реализована функция в стиле Arc для связывания закладок с вкладками. Теперь при нажатии на закладку или перетаскивании вкладки для создания закладки автоматически создается ссылка. Рядом с закладкой появляется значок ссылки, по которому можно щелкнуть, чтобы просмотреть все связанные вкладки для быстрого переключения или закрытия.
- Новая рождественская тема: в настройки добавлена мягкая и теплая рождественская тема, чтобы отпраздновать праздничный сезон.

🚀 Улучшения и исправления ошибок
- Защита от перетаскивания для создания закладок: при перетаскивании вкладки для создания закладки система теперь проверяет, существует ли уже закладка с таким же URL-адресом, и показывает диалоговое окно подтверждения для предотвращения дубликатов.

Для получения дополнительных примечаний к выпуску, пожалуйста, посетите: https://github.com/Tai-ch0802/arc-like-chrome-extension/releases
