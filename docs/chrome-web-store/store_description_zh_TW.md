# Arc-like Vertical Tabs & Bookmarks - 讓 Chrome 變身生產力神器 · 你的 Chrome 知識工作台

🔒 **預設本機** · 🔑 **免 API key（可自帶）** · 🤖 **內建 Gemini Nano AI**

Chrome 原生垂直分頁（2025）只把分頁改成垂直顯示。**這是一個完整的知識工作台**：統一管理分頁 + 書籤 + 閱讀清單，AI 加持（自動命名群組、分頁清理、滑鼠停留摘要、自然語言搜尋），Workspaces 一鍵 hibernate-restore 跨裝置同步，⌘K 命令面板，Bookmark Tools（標籤、重複偵測、失效連結）—— 預設在本機運算免 API key，也可自帶 key 接雲端模型（Gemini API / Claude / OpenAI 相容 / Ollama）。

厭倦了在數十個雜亂無章的分頁中迷失方向嗎？渴望 Arc 瀏覽器那般流暢優雅的垂直分頁體驗，卻又離不開 Chrome 的生態？
**Arc-like Vertical Tabs & Bookmarks** 是您的終極解答！🚀

我們將 Chrome 徹底改造，引入一個集 **垂直分頁**、**分頁群組**、**書籤管理** 於一體的現代化側邊面板。告別傳統的水平分頁列，重新掌控您的瀏覽器，體驗前所未有的專注與效率。


🌐 **官方網站**: [https://sidebar-for-tabs-bookmarks.taislife.work/](https://sidebar-for-tabs-bookmarks.taislife.work/)

---

### 🤖 Knowledge Workspace 2026 (新)
- **AI 自動命名群組**: 新建空白分頁群組時，Gemini Nano 自動產生 Emoji + 簡短標題。
- **AI 分頁清理建議**: 🧹 按鈕讓 AI 告訴你哪些分頁可以安全關閉。
- **AI 滑鼠停留摘要**: 停留分頁 2 秒顯示頁面一句話摘要。
- **AI 閱讀清單摘要記憶**: 加入閱讀清單時 AI 摘要一次並存本地，離線可看。
- **Ask AI 自然語言搜尋 (⌘K)**: 用一句話搜尋 tabs / bookmarks / reading list。
- **Workspaces**: 把分頁綁成工作區，一鍵 hibernate 與 restore，metadata 跨裝置同步。
- **Command Palette (⌘K / Ctrl+K)**: 統一搜尋 + 動作入口。
- **Bookmark Tools**: 🛠️ 多標籤 / 重複偵測 / 失效連結清理。
- **預設本機，雲端可選**: AI 開箱即跑在 Chrome 內建 Gemini Nano；也可自帶 API key（Gemini API / Claude / OpenAI 相容 / Ollama）。

## 🔥 核心亮點 (Key Features)

### 🔗 獨家創新：Linked Tabs (分頁與書籤的完美同步)
這是我們最強大的功能！當您從側邊欄打開書籤時，我們會自動建立 **「連結 (Link)」**。
- **避免太多重複分頁**：再次點擊該書籤的 link 圖示，會顯示已透過該書籤開啟的分頁，避免無腦新增大量相同分頁造成效能上損耗。
- **雙向同步**：分頁關閉時，書籤狀態自動更新；書籤刪除時，關聯分頁智慧處理。
- **視覺回饋**：書籤旁會顯示精緻的連結圖示，讓您一眼掌握哪些書籤正在使用中。

### ⚡️ 智慧渲染 (Smart Rendering)
書籤多到爆炸？別擔心！
- **動態渲染技術**：採用高效的 Dynamic Rendering 機制，棄用傳統的 Virtual Scrolling，在保持效能的同時大幅提升相容性。
- **流暢體驗**：即使擁有大量書籤，側邊欄依然反應靈敏。

### 🪟 跨視窗管理 (Cross-Window Management)
- **總覽所有視窗**：不只是當前視窗，您可以在側邊欄下方看到所有開啟中的 Chrome 視窗與分頁。
- **跨視窗搜尋**：搜尋功能全面支援跨視窗檢索，快速切換到任何已開啟的分頁。
### 🔍 專業級搜尋 (Advanced Search)
不只是搜尋，是「瞬間找到」。
- **多關鍵字過濾**：支援空白分隔 (例如：「google docs work」)，精準定位目標。
- **Domain 搜尋**：直接輸入網域 (如 `github.com`)，快速篩選特定來源的分頁與書籤。
- **智慧高亮**：即時標示匹配關鍵字，視覺焦點一目了然。
- **全區塊覆蓋**: 過濾、關鍵字反白與網域比對現在涵蓋側邊欄所有區塊——包含新的「封存」區。

### 🗂️ 統一工作空間 (Unified Workspace)
- **垂直分頁**：讓標題完整顯示，不再被壓縮成小圖示。
- **原生群組支援**：完美整合 Chrome 分頁群組 (Tab Groups)，顏色、名稱同步顯示。
- **自訂視窗命名**：可為每個視窗設定專屬名稱（如「工作」、「娛樂」），多視窗管理更清晰。
- **拖曳管理**：直覺的 Drag & Drop，輕鬆在分頁、群組、書籤資料夾之間移動項目。
- **拖曳即存**：將分頁拖入書籤區，立即收藏；將書籤拖入分頁區，立即開啟。
- **閱讀清單 & RSS**：您的個人文章收藏中心。與 Chrome 原生閱讀清單同步，訂閱 RSS 頻道，並透過智慧去重與排序功能統一管理。

### 🚦 分頁自動駕駛 (Tab Autopilot) (新)
- **自動分組規則**: 在分頁上按右鍵設定一次，之後同網站的新分頁在載入瞬間自動進入指定群組；設定頁提供完整規則管理，並跨裝置同步。
- **AI 自動成組 (選用)**: 沒有規則命中時，連續開啟同網站的多個分頁會自動聚成一組並由 AI 命名。
- **自動封存**: 閒置超過門檻（預設 12 小時）的分頁自動收進側邊欄新的「封存」區——釘選、群組內、使用中、播放中的分頁一律不動，每輪皆可一鍵復原。
- **稍後再看 (Snooze)**: 右鍵「稍後再看…」，分頁先關閉、時間到自動重開——1 小時後、今晚、明天早上或下週一。

### 🎨 極致美學與個人化 (Premium Design & Customization)
- **自訂背景圖片**：打造專屬工作空間！支援上傳圖片或輸入 URL，並可精確調整 **不透明度** 與 **模糊度**，讓側邊欄完美融入您的風格。
- **全新設定面板 UI**：現代化的 **手風琴 (Accordion) 收合佈局**，讓設定項目分類更清晰，操作更直覺、更清爽。
- **自訂主題顏色**：打破預設框架！現在您可以自由調整背景色、強調色與文字顏色，完全掌控視覺呈現。
- **專注模式**：深色主題搭配精心調校的對比度，有效減少眼睛疲勞。
- **自動展開**：拖曳時懸停於資料夾上方，自動為您展開路徑。
- **智慧懸停**：操作按鈕僅在需要時出現，保持介面乾淨無干擾。

---

## ⌨️ 全面鍵盤導覽與快捷鍵 (Full Keyboard Navigation)
- **原生級體驗**：使用 `Arrow Up` / `Arrow Down` 在分頁與書籤間流暢移動。
- **細節操作**：使用 `Arrow Left` / `Arrow Right` 可直接操作項目內的按鈕（如關閉、加入群組）。
- **高效率快捷鍵**：
    - `F2`：快速重新命名任何分頁或書籤。
    - `Delete`：一鍵刪除不需要的項目。
    - `Cmd/Ctrl + I`：快速開關側邊欄。
    - `Opt/Alt + T`：在當前分頁旁新增分頁。
- **搜尋整合**：在列表頂端按 `Up` 可直接聚焦搜尋框；在搜尋框按 `Down` 可直接進入結果列表。
- **無縫接軌**：側邊欄開啟後，按任一方向鍵即可自動開始導覽。

---

## 隱私與安全
我們重視您的隱私。本擴充功能 **預設完全在本機運作**，您的書籤、瀏覽紀錄與分頁資訊 **絕不會** 上傳至我們的伺服器；若您自行設定雲端 AI 供應商，AI 請求只會以您自己的 API key 直接送往該供應商。

---

**立即安裝，將您的 Chrome 升級為生產力巨獸！🌟**

---

## 開源與社群
這是一個開源專案。如果您有興趣參與維護，歡迎訪問我們的 GitHub 儲存庫：
https://github.com/Tai-ch0802/arc-like-chrome-extension
