...
- **모든 섹션 지원**: 필터링, 키워드 하이라이트, 도메인 일치가 새 ‘보관함’을 포함한 사이드바의 모든 섹션에서 작동합니다.
...
### 🚦 탭 오토파일럿 (Tab Autopilot) (신규)
- **자동 그룹 규칙**: 탭에서 마우스 오른쪽 버튼으로 한 번만 설정하면, 이후 같은 사이트의 새 탭이 로드되는 순간 지정한 그룹으로 자동 이동합니다. 설정 페이지에서 규칙을 관리하고 기기 간 동기화됩니다.
- **AI 자동 그룹화 (선택)**: 일치하는 규칙이 없을 때 같은 사이트의 탭을 연속으로 열면 자동으로 그룹화되고 AI가 이름을 지어줍니다.
- **자동 보관**: 임계값(기본 12시간)을 넘겨 방치된 탭은 사이드바의 새 ‘보관함’으로 자동 이동합니다. 고정·그룹·사용 중·재생 중인 탭은 건드리지 않으며 매번 원클릭으로 되돌릴 수 있습니다.
- **스누즈 (Snooze)**: ‘나중에 보기…’로 탭을 지금 닫고 예약 시간에 자동으로 다시 엽니다 — 1시간 후, 오늘 밤, 내일 아침, 다음 주 월요일.
