當前最新版本 - 1.7.0
✨ 新功能 (New Features)
- 書籤與分頁關聯功能：實作了 Arc 風格的書籤與分頁關聯功能。現在，點擊書籤或拖曳分頁建立書籤時，會自動建立連結。書籤旁會顯示關聯圖示，點擊後可查看所有已連結的分頁，並進行快速切換或關閉。
- 新增聖誕節主題：為迎接佳節，在設定中新增了一套柔和、溫暖的聖誕主題配色。

🚀 改善與錯誤修復 (Improvements & Bug Fixes)
- 拖曳防呆機制：當拖曳分頁來建立書籤時，系統現在會檢查書籤是否已存在，並彈出確認視窗，避免重複建立。
