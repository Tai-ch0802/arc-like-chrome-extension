...
key_files:
  ...
  - file_path: modules/newswire/tgAdapter.js
    description: "[快訊] Telegram adapter（BASE-018 TG1/TG2b）。**在 offscreen document 執行**（GramJS 客戶端狀態機;SW 端見 tgProxyAdapter）。經 DI 注入（createClient/NewMessage 可 fake 測）;實作同一 Adapter 介面＋狀態詞彙。connect→await createClient（async,dynamic import bundle）→client.connect()→逐頻道 getEntity→addEventHandler(NewMessage{chats})→新訊息以 {message,channel} 交 onRaw。await 窗口(dynamic import)內 disconnect 會 recheck stopped 拆掉剛建的 client 收手（避免 orphan/false connected）。classifyTgError 純函式（FLOOD_WAIT 遵守秒數／session 失效→needs-key 終止不迴圈／其餘退避）;eventChatId 對應頻道 meta。"
