...
key_files:
  ...
  - file_path: modules/newswire/tgAdapter.js
    description: "[快訊] Telegram adapter（BASE-018 TG1/TG2b）。**在 offscreen document 執行**（GramJS 客戶端狀態機;SW 端見 tgProxyAdapter）。經 DI 注入（createClient/NewMessage 可 fake 測）;實作同一 Adapter 介面＋狀態詞彙。connect→await createClient（async,dynamic import bundle）→client.connect()→逐頻道 getEntity→addEventHandler(NewMessage{chats})→新訊息以 {message,channel} 交 onRaw。await 窗口(dynamic import)內 disconnect 會 recheck stopped 拆掉剛建的 client 收手（避免 orphan/false connected）。classifyTgError 純函式（FLOOD_WAIT 遵守秒數／fatal 分 reason:session 失效→needs-login、api_id 問題→needs-key,終止不迴圈／其餘退避）;eventChatId 對應頻道 meta。**BASE-023**:teardownClient 統一拆除捨棄的 client（destroy 優先——teleproto autoReconnect 預設開,只 disconnect 的殘骸可能自行重撥與新 client 同 auth key 併發）;disconnect() 回傳拆除 promise,重建端（tgOffscreenController）await 它串行化新舊連線。"
  - file_path: modules/newswire/tgClient.js
    description: "[快訊] GramJS 客戶端工廠（BASE-018 TG2b）。**只在 offscreen 執行**（DOM context 支援 dynamic import;MV3 SW 不支援）。createTgClient/getNewMessage 動態 import lib/telegram.bundle.js（只在建 client 時,module 快取只載一次;import() reject 時清 _modPromise 讓重連可重試）。底層 teleproto（MIT,tools/telegram-bundle）。"
  - file_path: modules/newswire/tgProxyAdapter.js
    description: "[快訊] SW 端 Telegram proxy（BASE-018 TG2b）。GramJS 在 offscreen,此 proxy 實作同一 Adapter 介面但不持有 GramJS:connect→ensureOffscreenDocument+post tg:connect{cfg};disconnect→post tg:disconnect;isAlive=lastStatus==='connected';setRemoteStatus 收 offscreen 回報狀態透傳 setStatus。ensureOffscreen 期間 disconnect 收手不 post。"
  - file_path: modules/newswire/tgOffscreenController.js
    description: "[快訊] offscreen 端 tg 控制（BASE-018 TG2b）。從 offscreen.js 抽出以可單元測。管理 tgAdapter 生命週期、generation guard（dynamic import 窗口內 disconnect 不留 orphan——與 tgAdapter 內部 stopped guard 不同層）、tg:ping 回 {alive,hasAdapter,status}。DI:post/loadAdapter/loadClient（生產動態 import tgAdapter/tgClient）。offscreen.js 一次性載入後純 delegate。**BASE-022:resolveChannel 集中於此——offscreen 是唯一被允許用 session 連線的地方**。MTProto 對同一 auth key 的併發連線會直接作廢 session(AUTH_KEY_DUPLICATED);原設計讓 options 自建短命 client 做頻道解析,與此處常駐收訊連線併發,使用者連續加幾個頻道就被踢掉 session。兩條路徑都保證同時只有一條連線:常駐連線活著→借用 adapter.resolveChannel;未啟用(無常駐連線)→臨時建 client、finally 必 disconnect。unit 以 clientBuilt===0 鎖住「借用時不另建連線」。任何需要 session 的新操作都必須走此處,勿在 options/sidepanel 自建 client。**BASE-023:重建不重疊**——connect 於 adapter 已存在時,經 teardownDone 鏈 await 舊 client 拆除完成（await 後 recheck generation）才建新連線;resolveChannel 遇 adapter 暫為 null（重建拆除窗口/剛 disconnect）也先等鏈清空再判斷,不誤建與垂死連線併發的臨時 client。spec: docs/specs/fix/BASE-022_tg-session-duplicate/、docs/specs/fix/BASE-023_tg-session-roaming/。"
  - file_path: modules/newswire/tgLoginController.js
    description: "[快訊] Telegram 登入控制（BASE-018 TG2c）。在 **options 頁**(DOM context)執行——client.start 需互動 prompt(驗證碼/2FA),GramJS dynamic import(只登入時載)。login(client.start{phoneNumber,phoneCode,password}→session.save+getMe)/logout(client.logOut 遠端撤銷)/resolveChannel(getEntity 防仿冒確認)。DI loadGramJS 可 fake 測。安全:phone/驗證碼/2FA 一律 caller(UI)透過 callback 提供,不代跑;session≈完整帳號存取權,由 caller 存 storage＋風險告示。UI 在 options.js renderTgCard。"
  - file_path: modules/offscreenManager.js
    description: "[基礎] 共用 offscreen document 生命週期（BASE-018 TG2b）。Chrome 限單一 offscreen document,RSS(DOM_PARSER)與 tg(GramJS 常駐 WS)共用同一個 → ensureOffscreenDocument 集中單例 guard(try/finally 確保建立失敗歸零可重試);hasOffscreenDocument 供 watchdog 探活。原 rssManager 內一份抽出共用。"
  ...
  - file_path: modules/newswire/feedManager.js
    description: "[快訊] SW 編排核心（BASE-016）。單例持有各源連線；管線 raw→normalizer→dedupe→rules→eventBuffer→runtime 廣播（newswire:events/status）；newswire:getState/markSeen 訊息分派；config/keys（storage.local）變更→重建連線；keepalive 20s interval＋newswireWatchdog alarm（30s）喚醒重建；首次啟動 seed defaultNewswireConfig（來源全關＝零網路行為）。N3 加 exportLocalNewswireState/importMergedNewswireState 橋接（供 background Drive 同步鏈，import 內建 per-key canonical no-op guard）。N4 加 P0 系統通知（notifyP0：importance===0 且 prefs.notificationsEnabled 時 chrome.notifications.create，id 內嵌 event.id）＋handleNewswireNotificationClick（從 ring buffer 反查 url 開原文，耐 SW 回收）。TG2b：tg 改用 tgProxyAdapter（GramJS 移出 SW graph、在 offscreen）;watchdog 改 async,對 tg 以 tg:ping 探活;shouldReconnectTg 純函式據 **hasAdapter** 判定（無 adapter=offscreen 被 RSS 重建 adapter-less／tgConnect 失敗→重連;有 adapter 退避中 retrying/degraded→不干預,交 adapter 自癒,避免每 30s 砍退避重來加重 FLOOD_WAIT）;handleTgOffscreenMessage 收 offscreen 的 tg:raw(→管線,等 initPromise 避免 race buffer.init 丟事件)/tg:status(→proxy setRemoteStatus)。**BASE-023**:handleNewswireConfigChange 以 tgConfigIdentity 純函式（enabled/channels/keys.tg,刻意排除 updatedAt）判斷 tg 內容未變則保留既有連線——newswireConfig/newswireKeys 是粗粒度 key,改 rules/prefs/其他源 key 不再無謂拆掉 offscreen 常駐 MTProto 連線重撥（每次重撥都是同 auth key 短暫雙連線窗口）。"
  ...
  - file_path: modules/routing/rulesStore.js
    description: "[功能] 路由規則儲存層。chrome.storage.local 單 key routingRules {schemaVersion,rules[],tombstones}（≤50 條、刪除記 tombstone）；CRUD + reorder（重寫 order 並 bump updatedAt）+ sanitizeRuleInput 信任邊界共用；claimUrls：UI context 開分頁前送 routing:claim 給 background 並 await ack（fail-open，claim 失敗不得擋開分頁）。P4 增 exportRoutingState/importMergedRoutingState 作 Drive 同步橋接（rulesStore 對 Drive 無知，由 background routingSyncOnce 驅動，同 rssManager 分工）。"
  - file_path: modules/ui/routingRuleUI.js
    description: "[UI] 路由規則建立流程（BASE-022 P2）。promptCreateRuleForTab：右鍵選單「一律將此網站分到群組…」→ showGroupPickerDialog → addRule（網域規則）→ 立即套用一次（沿用引擎豁免：已分組/pinned 不動）→ toast + claimUndo（復原＝刪規則＋退群）。分頁右鍵選單本體在 contextMenuManager（http/https 分頁才顯示此項）。P3 增 initAiRoutingToast：監聽 SW 廣播 routing:aiGrouped（過濾本視窗）→ 可復原 toast → undo 送 routing:aiUndo；sidepanel 未開時靜默（SA D3）。"
  - file_path: modules/lifecycle/lifecycleLogic.js
    description: "[功能][純函式] 分頁生命週期決策核心（BASE-024 P1）。selectArchiveCandidates：閒置歸檔豁免鏈（pinned/群組內/active/audible/非http/lastAccessed 缺值保守不歸檔[Chrome 122+ fallback]/視窗唯一分頁——以「本輪歸檔後餘量」計算，兩候選同窗必留一）＋最舊優先＋單輪 cap 10；resolveSnoozeSlots：四固定時段（1h後/今晚20:00 已過跳明日/明天09:00/下週一09:00 週一當天跳下週）；isWakeExpired。常數 ARCHIVE_MAX_ITEMS=500。無 chrome/Date.now，時間入參。"
  - file_path: modules/lifecycle/archiveStore.js
  ...
  - file_path: modules/lifecycle/lifecycleEngine.js
    description: "[Service Worker] 生命週期引擎（僅 background 載入，BASE-024 P1）。lifecycleHeartbeat 常駐 30min alarm（ensure 僅於不存在時建立——重建會重置週期，SW 頻繁喚醒會無限延後）：sweep 過期 snooze（兜底 FR-4.04）＋掃描歸檔（開關關閉只停掃描）；歸檔「批次寫入成功→逐一關閉→關閉失敗補償移除」、喚醒「先開分頁→後刪記錄」——雙向維持 URL 任一時刻至少存在一處；lifecycleWake_<id> one-shot 喚醒＋snooze:<tabId> 通知（點擊聚焦，通知分派鏈以 prefix 認領）；廣播 {type:'lifecycle:archived'} 供 sidepanel toast（P2 接收）。閾值寬容讀取任何正數＝E2E 天然縫隙。onAlarm 分支在 background.js dispatcher（fall-through 事故防範）。"
  - file_path: modules/routing/routingRulesSyncLogic.js
    description: "[功能][純函式] 路由規則跨裝置合併邏輯（BASE-022 P4，比照 rssSyncLogic 契約：無 chrome/Date.now、時間由 now 傳入、對固定 now 交換律+冪等）。單檔 routing-rules-sync.json：per-rule updatedAt-LWW（平手 JSON 字串序決定性 tiebreak）、tombstone deletedAt≥updatedAt 隱藏（較新編輯復活）+ union + GC 60d、合併後依 (order, createdAt) 重排；buildRoutingRulesPayload（頂層 updatedAt/deviceId 為簿記）、canonicalizeRoutingRules/routingStatesEqual（no-op write guard，比 identity 子集）、isRoutingSchemaTooNew（較新 schema 跳過該輪、拒絕降級回寫）。已知天花板：雙裝置同時 reorder 收斂 per-rule 近似解。"
  - file_path: modules/routing/routingEngine.js
    description: "[Service Worker] 路由引擎（僅 background 載入，BASE-022 P1）。tabs.onCreated/onUpdated 偵測分頁首次 http 導航（免 webNavigation 權限），依規則入群/建群；豁免鏈：claim（onCreated 以 pendingUrl 綁定，redirect-proof）→ 無 candidate 不路由（僅路由引擎在場時出生的分頁）→ 全域開關 → 啟動還原排除 → 已分組/pinned。暫態集中單一 chrome.storage.session key routingSessionState，單一寫者 + promise chain 序列化（SW 終止喚醒一致、瀏覽器重啟歸零）；規則與開關快取以 storage.onChanged 失效。每分頁最多判定一次、手動拖出永不搬回。P3 增 AI Auto Routing 分支（opt-in aiAutoRoutingEnabled）：規則未命中時 decideAiRouting 決策 join（存活 AI 群組直接入、並更新 lastNav 維持連續性語意）/create（回溯納入前一分頁，需 live re-check）/none；建組以網域為暫名＋隨機色，AI 命名走 fire-and-forget scheduleAiNaming（不阻塞 mutate chain、寫入前 re-check 暫名防覆寫手動改名、AI 不可用保留網域名）；廣播 routing:aiGrouped 給 sidepanel toast；routing:aiUndo 以 groupId 查詢現任成員整組解散（join 進來的也算）＋記 30min 冷卻；tabGroups/windows onRemoved 清 aiGroups 映射。"
  ...
  - file_path: modules/workspace/workspaceUI.js
    description: "[UI] Workspace 切換器與管理介面。M3 重構後:工作區列為單一 #workspace-switch-btn(名稱置中、無 emoji)→開管理 dialog;rename/delete/cloud 用 Material Symbols。Arc 切換改版:performSwitch 非破壞性(focus-or-open)、移除確認對話框與 unbound auto-save;auto-snapshot/window cleanup/prune 全部移至 workspaceLifecycle.js(background);initWorkspaceUI 接受 onWorkspacesChanged callback,workspace storage 變更時供 sidepanel 連動刷新 Other Windows 標題(顯示工作區名稱)。BASE-021 新增 handleOverwrite(以目前視窗覆蓋既有工作區):按鈕只在該工作區未綁定本視窗時顯示,**先 setActiveWorkspace 再 snapshotIntoWorkspace**(順序不可換——未綁定的空窗期會讓 Drive pull 的 keepLocalSnapshot 失效而回捲覆蓋成果);寫入前先以 buildSnapshotFromTabs 預算快照,因 snapshotIntoWorkspace 對「空快照擋下/內容相同跳過/實際寫入」都回傳同一個 ws 無法分辨,預算同時供確認框顯示 N→M 分頁數並攔截全 chrome:// 視窗的靜默無效。spec: docs/specs/feature/BASE-021_workspace-overwrite/。"
