...
- [sidepanel.css]: 
    - 為 `#tab-list`, `#bookmark-list`, `#other-windows-list` 添加 `content-visibility: auto`。
    - **[Fix]** 未設定頁 Dialog 增加 `max-height` 與 `overflow-y` 設定，解決內容過長無法顯示的問題。
...
- **CSS :has() 應用**: 利用 `:has(.modal-custom-content)` 選取器來針對特定的設定頁 Dialog 實作 Sticky Header/Footer 效果，而在不改動 JS 的前提下優化了捲動體驗。
