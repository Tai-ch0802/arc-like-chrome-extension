...
> **本專案脈絡**：arc-like-chrome-extension 是 vanilla JS ES Modules + 純 CSS（無 Tailwind / React / Vue），故 stack guidelines 的可移植性有限。實際採用時請把 Tailwind utility 對應回語意化 class + plain CSS；資料庫的核心價值仍在 `colors.csv` / `typography.csv` / `ui-reasoning.csv` / `styles.csv` 的設計決策參考。
